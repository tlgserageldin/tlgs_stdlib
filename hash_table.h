#ifndef Q_HASH_TABLE_H
#define Q_HASH_TABLE_H
...
static int ht_destroy(ht **table);

// returns null if not found, or missuse
static void *ht_search(ht *table, s8 key);

// non-zero return on error
// 2^53 max entries
static int ht_insert(ht *table, s8 key, void *value);

// non-zero error
static int ht_remove(ht *table, s8 key);

#ifdef Q_HASH_TABLE_IMPLEMENTATION
...
static void *ht_search(ht *table, str key) {
...
// non-zero if failure
static int ht_insert(ht *table, str key, void *value) {
...
static int ht_remove(ht *table, str key) {
  if (!is_valid_str(key) || !_ht_is_valid(table)) {
    return 2;
  }
  return 0;
...
#endif // Q_HASH_TABLE_IMPLEMENTATION
#endif // Q_HASH_TABLE_H
